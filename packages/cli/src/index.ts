...
export function createProgram(): Command {
  ...
  program.command("list").description("List connected DYMO printers").action(async () => {
    ...
    for (const printer of printers) {
      console.log(`${printer.device.name} (${printer.path}) ${printer.serialNumber ?? ""}`.trim());
    }
  });
  ...
  program.command("status").description("Show printer status").action(async () => {
    const printer = await openPrinter();
  ...
  setup.command("linux").description("Print Linux setup instructions").action(() => {
