  ...
  <section class="live-demo">
    ...
    <div class="actions">
      ...
      <p v-if="!printer" class="webusb-note">
        Printing requires <strong>Chrome</strong> or <strong>Edge</strong> with WebUSB.
        The preview works in any browser.
      </p>
    </div>
  </section>
</template>
...
.dot-idle      { background: var(--vp-c-text-3); }
...
.btn-disconnect:hover { color: var(--vp-c-text-1); }
...
.btn-connect:not(:disabled):hover { opacity: 0.88; }
...
.btn-print:not(:disabled):hover { opacity: 0.88; }
...
.status-error { color: var(--vp-c-danger-1, #f44); }
